...
export default function Emi() {
  ...
  return (
   ...
   <main className="max-w-7xl mx-auto p-4 grid gap-6 sm:grid-cols-1 md:grid-cols-2">

  {/* Loan Details Section */}
  <motion.section
    layout
    className="rounded-2xl bg-white shadow-lg border border-yellow-100 p-4 w-full"
    initial={{ opacity: 0, y: 30 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true, amount: 0.3 }}
    transition={{ type: "spring", stiffness: 70, damping: 16, delay: 0.15 }}
  >
    <motion.h2 className="text-lg font-bold mb-4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, type: "spring", stiffness: 80 }}>
      Loan Details
    </motion.h2>
    ...
    <motion.div className="space-y-4" initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.08 } } }}>
      ...
      {/* Info Box */}
      <motion.div className="flex gap-2 text-sm text-neutral-700 bg-yellow-50 border border-yellow-200 p-3 rounded-2xl"
        variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
        <FiInfo className="mt-0.5 shrink-0 text-yellow-600" />
        <p>EMI uses the standard formula with monthly compounding. Edit values to see results update instantly.</p>
      </motion.div>
    </motion.div>
  </motion.section>
  ...
  >
    <motion.h2 className="text-lg font-bold mb-4"
      initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, type: "spring", stiffness: 80 }}>
      Results
    </motion.h2>
    ...
    <div className="mt-5">
      <motion.button
        ...
        className="inline-flex items-center gap-2 rounded-2xl border px-3 py-2 text-sm hover:bg-yellow-50 active:scale-[.98] transition w-full sm:w-auto justify-center"
      ...
      <AnimatePresence initial={false}>
        {showSchedule && (
          <motion.div key="schedule" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ type: "spring", bounce: 0, duration: 0.5 }} className="overflow-hidden">
            
            <motion.div className="mt-4 max-h-80 xs:max-h-96 overflow-auto rounded-2xl border border-yellow-100 bg-yellow-50 w-full">
